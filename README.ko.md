...
- **인코딩 자동 감지**: UTF-8을 우선 확인하고, 엑셀의 "유니코드 텍스트" 저장 형식인 UTF-16(BOM 유무 모두)도 자동 감지합니다. 레거시 인코딩(`cp949`, `cp1252`)은 깨진 문자가 가장 적은 쪽을 선택하여 한국어 엑셀 파일도 글자 깨짐 없이 처리합니다. 탭 구분 파일은 구분자 칸에 `\t`를 입력하면 됩니다.
